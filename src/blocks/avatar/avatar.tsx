...
export const Avatar = React.forwardRef(
  (props: AvatarProps, ref: ReactRef<HTMLSpanElement>) => {
    ...
    return (
      ...
      >
        <span className="trident-avatar-bg" />
        {!showText && (
          <img
            ref={imgRef}
            className={clsx(
              "trident-avatar-img",
              `trident-avatar--${getState}`,
              {
                "trident-avatar-ready": ready,
              }
            )}
            src={src}
            alt={alt}
        ...
        {showText && !icon && text && (
          <span className="trident-avatar-text">{safeText(text)}</span>
        )}
        {icon && <span className="trident-avatar-icon">{icon}</span>}
      </StyledAvatar>
    );
  }
);
